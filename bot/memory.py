...
class MemoryStore:
    ...
    def __init__(self, pool: asyncpg.Pool, http: httpx.AsyncClient, config: Config):
        ...
        # (the bot itself), injected by the bot wiring. None in maintenance/headless contexts,
        # where every author counts. See `search_claims`.
    ...
    async def search_claims(self, query: str, k: int) -> list[RecalledClaim]:
        ...
        async with self._pool.acquire() as conn:
            ...
            # The bot author (when set) is excluded from the agreement count via
            # ``IS DISTINCT FROM`` — which counts every (non-null) author when ``bot_author`` is
            # None, so maintenance/headless recall is unaffected. Bot-authored claims still group
            # and surface (recency/object aggregate over all asserters); they just don't inflate
            # corroboration, so a human asserting the same value always outranks a bot-only one.
