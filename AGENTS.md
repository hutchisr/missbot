...
- World-knowledge tool (when `memory_enabled`): `search_memory` (agreement-ranked recall — the most-agreed value per subject+predicate with its `agreed by N` count and any competing values, fenced as untrusted data). Notes are ingested automatically (no model write tool). Not given to the auto-agent
