...
- Every model-provider request identifies the app with `HTTP-Referer: rad://zLseUdKik1qrsiTonrjSoPGYbC6g`, versioned `User-Agent: Missbot/<version>`, and `X-OpenRouter-Title: missbot-<version>` headers. This covers reply, ACP, autonomous-post, social-scoring, mem0 extraction and embedding, and image-generation calls
