...
- `trusted_user_ids` (default `[]`): user ids (matched by stable id, like `social_credit_unrestricted_user_ids` — **not** handle, since ids aren't spoofable across federation) whose ingested note claims enter at the promotable **`primary`** tier instead of `user`. Source `kind` stays `user` (so retraction-by-handle still works); only the claim's `trust_tier` is elevated. A single trusted user is still `asserted`; two promotable sources asserting the same value reach `believed`. This is the main live path that re-activates corroboration/`believed` (the removed web-search ingestion was the other). Extraction admission gate + `looks_sensitive` PII backstop still apply
